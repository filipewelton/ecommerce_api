defmodule EcommerceApi.Customers.Update do
  ...
  def call(params) do
    with :ok <- has_id(params),
         {:ok, customer} <- Get.by_id(params["id"]),
         {:ok, changeset} <- changeset(customer, params),
  ...
  defp changeset(struct, params) do
    case Customer.changeset(struct, params) do
