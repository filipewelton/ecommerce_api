defmodule EcommerceApiWeb.ManagersController do
  ...
  def sign_up(conn, params) do
    payload = Map.put(params, "role", :manager)
    ...
    with {:ok, user} <- create_store_user(payload) do
      ...
      {:ok, token} = create_user_token(:manager, claims)
  ...
  def sign_in(conn, params) do
    ...
    with {:ok, user} <- get_store_user_by_email(:manager, email),
